...
app.post('/api/shorturl', async function(req, res) {
  ...
  extractedUrl = urlExtractor(inputUrl);

  if (extractedUrl) {
    dns.resolve(extractedUrl, async (err, address, family) => {
      if (err) {
        ...
        try {
          ...
          if (findOne) {
            ...
            // if it is not in database yet, create  a  new one and respond with the result
            findOne = new URL({
              original_url: url,
              short_url: shortUrl
            })
